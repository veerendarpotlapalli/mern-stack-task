...
// Sales Order
export const getSalesOrderData=()=>api.get(url.GET_SALES_ORDER );
export const getSingleSalesData=(payload)=>api.get(url.GET_SINGLE_SALES_ORDER+`/${payload?.id}`);
export const getVerifyListData=(payload)=>api.get(url.GET_VERIFY_LIST +`?search=${payload?.searchText}`);
export const getStartPicking=(payload)=>api.update(url.START_PICKING_PROCESS+`/${payload?.id}`);
export const getScanProductWithBarCode=(payload)=>api.update(url.SCAN_WITH_BARCODE,payload)
// Login Method
...
export const createEmploye = (payload) => api.create(url.CREATE_EMPLOYE, payload);

export const editEmploye = (payload) => api.patch(url.UPDATE_EMPLOYE + `/${payload?.id}`, payload);

export const deleteEmploye = (payload) => api.delete(url.DELETE_EMPLOYE + `/${payload}`);
